//! 重要资料命令：导入（明文入库）/ 列表 / 删除 / 导出。
//! 资料文件不加密，仅账号密码需要主密钥（见 accounts.rs）。
...
/// 导入：选择本地文件 → 明文存入 vault → 记录元数据（后台线程执行，避免卡 UI）
#[tauri::command]
...
fn import_document_sync(st: Arc<AppState>, src_path: &str) -> AppResult<Document> {
    ...
    // 明文保存：UUID + 原扩展名（便于直接打开）
    let ext = Path::new(&file_name)
        .extension()
        .and_then(|e| e.to_str())
        .map(|e| format!(".{e}"))
        .unwrap_or_default();
    let store_name = format!("{}{}", uuid::Uuid::new_v4(), ext);
    fs::write(vault_dir.join(&store_name), raw)?;
...
/// 导出：复制明文文件到用户选择的路径（后台线程执行）；
/// 旧版加密资料（.enc）需要解锁后用主密钥解密导出。
#[tauri::command]
...
fn export_document_sync(st: &crate::state::AppState, id: &str, dest_path: &str) -> AppResult<()> {
    ...
    let src_path = st.vault_dir.lock().unwrap().join(&doc.file_path);
    if doc.file_path.ends_with(".enc") {
        // 旧版加密资料：需解锁后解密
        ...
        fs::copy(&src_path, Path::new(dest_path))?;
    }
    Ok(())
}
