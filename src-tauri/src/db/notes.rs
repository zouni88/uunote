//! 笔记 CRUD
//!
//! 每篇笔记在创建时选定编辑模式（自由画布 / Markdown / 富文本），创建后锁定：
//! - mode 记录笔记类型（freeform / markdown / richtext），不可切换
//! - content 存放该模式下的内容（画布场景 JSON / Markdown 源码 / 富文本 HTML）
//!
...
pub struct Note {
    pub id: String,
    pub title: String,
    /// 编辑模式（freeform / markdown / richtext），创建后锁定
    #[serde(default = "default_mode")]
    pub mode: String,
    /// 当前模式下的内容（画布场景 JSON / Markdown 源码 / 富文本 HTML）
    #[serde(default)]
    pub content: String,
    pub pinned: bool,
...
const COLUMNS: &str = "id, title, mode, content, pinned, group_id, created_at, updated_at";
...
pub fn create(conn: &Connection, title: &str, mode: &str, group_id: Option<String>) -> AppResult<Note> {
...
pub fn update(conn: &Connection, note: &Note) -> AppResult<Note> {
    ...
    tx.execute(
        "UPDATE notes SET title = ?1, mode = ?2, content = ?3, pinned = ?4, group_id = ?5, updated_at = ?6,
         v_lamport = ?7, v_device = ?8 WHERE id = ?9",
        (
            &updated.title,
            &updated.mode,
            &updated.content,
